...
[![CI](https://github.com/Angana007/Med-RAG/actions/workflows/ci.yml/badge.svg)](https://github.com/Angana007/Med-RAG/actions/workflows/ci.yml)
...
- **Hybrid retrieval** — deterministic SQL lookups for structured facts + FAISS vector search for unstructured clinical notes
- **Live safety guardrails** — PHI pattern scanning, cross-patient leak detection, and a retry-then-block policy on every request
- **Scope guardrails** — imaging and billing/legal/administrative questions rejected before any retrieval or LLM call
- **Built-in evaluation framework** — RAGAS-style metrics (faithfulness, answer relevance, groundedness, hallucination flags, safety, latency) shared between live and offline code paths
- **100% local inference** — Phi-3 via Ollama + sentence-transformers; no patient data ever leaves the machine
- **Production scaffolding** — API-key auth, per-IP rate limiting, `/health` and `/metrics` endpoints, structured logging
- **Tested & containerized** — pytest suite with GitHub Actions CI, Dockerfile + docker-compose one-command startup
...
| `tests/test_retrieval.py` | Pytest suite — guardrails, MRD validation, SQL retrieval, safety checks, latency budget. Runs without a live LLM (see [Testing & CI](#-testing--ci)). |
| `.github/workflows/ci.yml` | GitHub Actions workflow — pytest + flake8 on every push and pull request. |
| `Dockerfile` / `docker-compose.yml` / `entrypoint.sh` | Container build for the API and a one-command full stack (API + Ollama). The entrypoint auto-builds the demo data stores on first startup — see [Quick Start (Docker)](#-quick-start-docker). |
...
✅ Together these **reduce hallucinations** via context-only prompting + `temperature=0` — measured **90% grounded-answer rate** (10/13 benchmark queries returned source-backed answers or principled refusals) — and keep answers PHI-safe and traceable, with a documented fallback path when something still goes wrong. Hallucination is mitigated and measured, not "eliminated" — no RAG system can honestly claim the latter.
...
The fastest way to run the full stack — FastAPI + Ollama — with one command:

```bash
docker compose up --build
```

Then pull the model into the Ollama container (first run only):

```bash
docker compose exec ollama ollama pull phi3
```

The API is now live at `http://localhost:8000` (Swagger UI at `/docs`). Try it:
...
- On first startup the API container **builds its own demo data stores** (SQLite DB + FAISS index) from the bundled `synthetic_patient_records.json` — this includes a one-time ~90 MB embedding-model download, cached in a named volume for later runs. Ollama's pulled models persist the same way.
- Set a real API key via `CLINICAL_RAG_API_KEY` in `docker-compose.yml` before any non-local deployment.
- Prefer a manual setup (venv + local Ollama)? See [Project Setup & Usage](#-project-setup--usage) below.
...
```
Review the generated report in `eval_reports/` before trusting the system's outputs — see [Evaluation Framework](#-evaluation-framework) above and the published numbers in [Performance Metrics](#-performance-metrics) below.

## ✅ Testing & CI

The `tests/` suite covers the parts of the pipeline that don't need a live LLM — so it runs fast locally and in CI:

- **Scope guardrails** — imaging and billing/legal queries are rejected before any retrieval, with the right error messages
- **MRD validation & retrieval hits** — invalid MRDs are rejected; the demo patient's records come back from the SQL layer
- **Safety checks** — PHI pattern scan, cross-patient leak detection, and hallucination flagging (`evaluation.py`'s pure functions)
- **Latency** — structured lookups stay within budget
...
GitHub Actions ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)) runs pytest + flake8 on every push and pull request — that's the badge at the top of this README. LLM-dependent quality metrics (faithfulness, relevance) are measured separately via `eval_runner.py`, since they require a live Ollama instance.

## 📈 Performance Metrics

Measured on the full 13-case benchmark (`eval_test_cases.json`) via `python eval_runner.py`, running Phi-3 locally through Ollama. Full per-case detail: [`eval_reports/eval_report_20260709_231050.json`](eval_reports/eval_report_20260709_231050.json).

| Metric | Score | How it's measured |
|---|---|---|
| Retrieval hit rate | **77%** (10/13) | Queries that retrieved usable context and produced an answer |
| Answer relevance | **0.815** / 1.0 | LLM-judged: does the answer address the question asked? |
| Faithfulness | **0.85** / 1.0 | LLM-judged: is every claim supported by the retrieved context? |
| Grounded answer rate | **90%** (9/10) | Answers that were source-backed or principled refusals (no hallucination flag) |
| Avg end-to-end latency | **20.8 s** | SQL + vector search + local Phi-3 generation, per query |
| Safety pass rate | **80%** (8/10) | Answers passing the PHI scan + cross-patient leak check on first generation |

**How to read these numbers:**

- **The 3 "missed" retrievals are mostly by design.** Two of the three non-answered cases were the billing and MRI edge-case queries that the scope guardrails are *supposed* to reject, and the third was an invalid-MRD validation test — the system refused rather than guessed, which is the intended behavior for a clinical assistant.
- **Grounded answer rate (90%) is the headline anti-hallucination number.** Context-only prompting + `temperature=0` means 9 of 10 answered queries were either directly source-backed or explicit "the context does not specify…" refusals; exactly one answer tripped the hallucination flag (numbers not present in the retrieved context).
- **The two safety "failures" were false positives from an intentionally aggressive detector.** The cross-patient leak regex flagged capitalized phrases like "Total Records" as potential foreign patient names. Erring toward over-flagging is the right default for PHI safety — the retry-then-block policy handles it — but tightening the name heuristic is on the roadmap.
- **Latency is dominated by local CPU inference.** Phi-3 generation plus the LLM-judged scoring both run on local hardware with no GPU; SQL lookups average ~1 ms. A GPU or a smaller quantized model would cut this substantially.
