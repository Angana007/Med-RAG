...
Structured (SQL) storage layer for the Clinical RAG Assistant.

Sets up a permanent SQLite database that stores patient/visit metadata
(name, gender, doctor, discharge date, document type, visit id) so the
system can quickly verify a patient exists and pull their entire visit
history before the AI starts answering questions. This is the "SQL Facts"
half of the hybrid retrieval strategy — see retrieval.py for how it's
combined with the FAISS "Vector Narratives" half.

Typical usage (also see setup_db.py, which just calls these two in order):
    from database import init_db, populate_db, get_patient_metadata
    init_db()                              # create the table if missing
    populate_db()                          # load *.json files in cwd into it
    get_patient_metadata("20001")          # -> list[dict], one dict per visit
"""
